...
\begin{figure}
	...
	\caption{Focus of the IR beam after passing through a centered $\pi$-plate. Left panel: numerical evaluation of \cref{eqn:diffraction_integral} using \cref{eqn:fresnel_paraxial_approx} and a measured input laser beam profile. Middle panel: TEM\textsubscript{01} mode. Right panel: lineouts at $x_p = 0$ comparing the numerical result to the TEM\textsubscript{01} mode.}
...
In some experiments, we use a phase mask to shape the IR beam profile at the focus. When designed correctly, the phase mask can convert a TEM\textsubscript{00} mode into a TEM\textsubscript{01}-like mode at the focus \cite{hagemanComplexAttosecondTransientAbsorption2020,camperHighRelativephasePrecision2019,camperTransverseElectromagneticMode2015,camperHighharmonicPhaseSpectroscopy2014,camperCombinedHighharmonicInterferometries2015}. The two IR spots are phase-locked to each other with a phase difference of $\pi$. If the beam is focused onto a gas plume, each intensity lobe will locally drive HHG, and the harmonics will spatially interfere in the far-field. Below, we will show how a binary phase plate can produce a TEM\textsubscript{01}-like mode at the focus. We have also used phase gratings to produce two lobes at the focus with a variable phase difference. For more details on the phase grating, see reference  \cite{hagemanComplexAttosecondTransientAbsorption2020} and \cite{camperHighRelativephasePrecision2019}.
...
\end{equation}
where $n$ is the refractive index of the glass. Ignoring Fresnel losses and material absorption, the phase plate has a complex transmittance of $\tau_{pp}$:
\begin{equation}
...
\end{equation}
%We assume a Gaussian input laser field with unit field strength and radius $\sigma$:
%\begin{equation}
...
\end{equation}
this is valid if $P$ is close to the optical axis. Using this approximation, \cref{eqn:diffraction_integral} was numerically integrated and evaluated at the focus \cite{vdovinLightPipesPython}. The input beam $E_{in}$ was imaged using a thermal camera and scaled up by the magnification factor of the telescope ($M=5/3$). The result is shown in \cref{fig:pi_plate_focus_sim}. We can see that there are two intensity lobes at the focus. The profile is Gaussian along the $x_p$-direction, and there are maxima located at:
\begin{equation}
...
\end{align}
For this reason, it is often said that the $\pi$-plate converts a TEM\textsubscript{00} beam into a TEM\textsubscript{01} mode at the focus. The exact intensity and the TEM\textsubscript{01} approximation for a centered $\pi$-plate are shown in \cref{fig:pi_plate_focus_sim}.