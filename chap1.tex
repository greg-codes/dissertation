...
%
%The invention of the laser quickly lead to the development of perturbative nonlinear optics. By varying the refractive index and absorptivity as a function of laser intensity, researchers were able to create light modulators which lead to the invention of phase locked laser cavity modes. Chirped pulse amplification lead to 
%
%The first laser had a coherence time of seconds, which meant that over $10^{15}$ cycles of light were emitted in perfect coherence. For this unprecedented control over light, Basov, Prokhorov and Townes were awarded the Nobel prize (1960s). The invention of the laser paved the way for nonlinear optics, for which Franken and Bloem
%
%The invention of the laser in the 1960s, followed shortly by the development of nonlinear optics, rapidly decreased the 
%
%the path from nano to attoseconds
%
%- ability to emit coherent light over $10^{15}$ cycles, via the discovery of the laser. Basov, Prokhorov and Townes awarded the nobel prize (1960s)
%
%- perturbative nonlinear optical effects. bloembergen awarded the 1981 nobel prize. lead to the creation of light modulators, which enabled phase locked laser cavity modes.
%
%- chirped pulse amplification (CPA) by Donna Strickland and G\'{e}rard Mourou (2018 nobel prize)
%
%- a second revolution in nonlinear optics, resulted in femtosecond laser pulses. this allowed for the observation of chemical bonds breaking by Zewail (1999 nobel prize)
%
%- near the turn of the millenium, laser pulses were down to their ultimate limit - a single laser cycle. 
%
%- ultrashort pulses lead to the study of nonperturabtive nonlinear optical effects such at above-threshold ionization (ATI) and high harmonic generation (HHG).
%
%- HHG enabled the creation of sub-fs trains of light pulses in the extreme ultraviolet (XUV) regime
%
%- further advances in HHG technology have lead to the creation of isolated attosecond pulses (IAP). the current world record is just 53 attoseconds \cite{li53attosecondXrayPulses2017}.
%
%- sub-as measurement \cite{ossianderAttosecondCorrelationDynamics2017}
%
%- 247 zeptosecond measurement \cite{grundmannZeptosecondBirthTime2020}
...
%citations:
%
%- electron dynamics using ATAS: \cite{ramaseshaRealTimeProbingElectron2016}
%
%- figure that shows transient absorption and its Fourier transform \cite{beckProbingUltrafastDynamics2015}
%
%- paper that talks about few-fs timescale dynamics in semiconductors \cite{piconAttosecondXrayTransient2019}
...
%$n^*$ is the effective principle quantum number, $l^* = n^* -1 $ is the effective orbital quantum number, and $w_{ADK}$ is the instantaneous ADK rate:
%\begin{equation}
...
%The dipole strength is assumed to follow $A_q \sim (1-\eta)I_0^5$, where $I_0$ is the intensity at the focus \cite{krauseCalculationPhotoemissionAtoms1992,kuhnELIALPSFacilityNext2017a}.
...
%The physical interpretation of the measurement depends on the type of sample and the order of the pulses. When the sample is a rarefied gas, it is helpful to think of the response in terms of an induced polarization. We consider the single-atom response:
%\begin{equation}
%\tilde{S}(\omega) = - 2 \Im \left[ \tilde{d}(\omega) \tilde{E}^*(\omega) \right]
%\end{equation}
%Here, $d$ is the time-dependent one-electron single-atom dipole moment, $E$ is the electric field, and the tilde denotes a Fourier transform at frequency $\omega$. The corresponding macroscopic polarization is the product of the density of the gas $\rho$ and the dipole response: $\tilde{P} = 2 \rho \tilde{d}(\omega)$. Therefore, the absorption cross section is
%\begin{equation}
%\sigma(\omega) = 8 \pi \alpha \omega \Im \left[ \frac{\tilde{d}(\omega)}{\tilde{E}(\omega)} \right]	
%\end{equation}
%where $\alpha$ is the fine structure constant.

%Many-body effects cannot be disregarded in a dense gas or in the condensed phase, so the interpretation becomes more involved.
...
The experiment is engineered so that part of the XUV spectrum is resonant with transitions between core shell states and the valence/conduction bands. These transitions correspond to photon energies between 20 and a few hundred electron volts, depending on the material and the initial state. Due to electron screening, the core states are effectively shielded from the external MIR field. Since the XUV field is weak, we only need to consider linear absorption, which is due to the imaginary part\footnote{Neglecting the real part of $\epsilon$ is justified in \cref{sec:real_imag_index}.} of the linear dielectric function, $\epsilon$ \cite{kaplanFemtosecondTrackingCarrier2018}:
\begin{equation}
...
\end{equation}
%
%The relatively simple form of \cref{eqn:phase-plate-transmission} allows for direct integration of \cref{eqn:diffraction_integral}. Below, we will perform this integral and evaluate the result at the focus ($z_p = f$). For the general case of a $\pi$-plate ($\phi=\pi$) with an arbitrary offset $y_0$, \cref{eqn:diffraction_integral} evaluates to:
%\begin{equation}
%\begin{aligned}
