...
A 1 $\mu$m longpass filter (Thorlabs FELH1000, $\text{OD}>5$) is positioned before the waveplate-polarizer assembly to filter out the OPA's visible parasitic wavelengths. This is neccessary to suppress short-wavelength excitation of condensed matter samples. The IR intensity incident on the sample is controlled by a motorized achromatic $\lambda/2$ waveplate (Thorlabs AHWP10M-1600, $\lambda = 1100 - 2000$ nm) and a pair of ultra broadband wire grid polarizers (Thorlabs WB25M-UB, each with a 1000:1 extinction ratio for $\lambda = 0.6 - 4 \text{} \mu \text{m}$) in the pump arm (see \cref{fig:beamline_schematic}). 

A pair of 4 degree fused silica wedges are used to finely control the XUV-IR delay. A stepper motor inserts the wedge into the beam, increasing the optical path length and providing about 1.5 ps total delay range. Coarse delay control is provided by a retroreflector on a manual translation stage. As a future upgrade, the retroreflector could be motorized to enable the study of ps-scale dynamics.

\textbf{say something about calibration of delay wedges here}
...
We use an Electrophysics PV320 thermal camera for daily alignment into the TABLe interferometer.\footnote{Special thanks to Eric Moore who wrote a LabVIEW program to semi-automate this process.} The camera is located between S and L4, and we use two irises (the \nth{1} is located between BS and CF, the \nth{2} is located just before S) to put Airy diffraction rings in the beam. Two motorized mirror mounts located on the TOPAS table control the pointing of the input beam. The alignment procedure is iterative and multi-step: first, the centroid of the full beam is calculated; second, the \nth{1} iris is  ...

\textbf{need to finish this description}
...
\begin{figure}
	\centering
	\includegraphics[width=0.75\textwidth]{figures/chap2/two-source-HHG.pdf}
	\caption{Two-source harmonic generation scheme. The pump beam is not used in this context, and the XUV refocusing optics are omitted. See text for details. Figure adapted from \cite{camperTransverseElectromagneticMode2015}.}
	\label{fig:two-source-HHG}
	% chap 2 figures powerpoint.
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=0.75\textwidth]{figures/chap2/multi-order-PiPlate.pdf}
	\caption{Two-source harmonic spectra showing multiple grating diffraction orders. Diffraction orders are identified as follows: $m=1$ in pixel range 1250 - 1550; $m=2$ in range 730 - 1070, $m=3$ in range 320 - 670, $m=4$ in range 30 - 300. Top panel: detector image of HHG spectra showing spatial interference of the two sources. Bottom panel: Fourier transform of top panel along spatial dimension, showing the dominant spatial frequency for each harmonic.}
	\label{fig:multi-order-PiPlate}
	% figure made in \Python Scripts\Spectrometer\test\2017_09_19.py
\end{figure}

It can be difficult to visually identify matched pairs of harmonic orders. We can use the divergence axis of the spectrometer to help with this process. We do this by inserting a phase mask into the laser beam upstream of the generation lens. This optic is designed to imprint a spatially varying phase delay on the laser pulse. If the phase mask is a $0-\pi$ step function, or a $0-\pi$ grating, it can be shown that the laser will focus to a TEM\textsubscript{01} like-profile in the focal plane \cite{hagemanComplexAttosecondTransientAbsorption2020}. Furthermore, these two laser spots will be phase-locked to each other, with a separation of roughly $\lambda f / a$, where $f$ is the focal length and $a$ is the input beam size. If a gas jet is placed in the focal plane, then we will have two phase locked harmonic sources which will interfere in the far-field with spatial frequency $(2q+1)f/a$. This is schematically shown in \cref{fig:two-source-HHG}. By performing a Fourier transform along the spatial dimension, we can identify matching pairs ($m=1,2$) of harmonics. Once we have identified these pairs, we can apply the method described above to find the absolute harmonic order. For more details on two-source harmonic generation, see Stephen Hageman's dissertation \cite{hagemanComplexAttosecondTransientAbsorption2020}.
...
\begin{figure}
	...
	\caption{Fano resonances used for calibration. Top panel: XUV spectra with and without argon gas in the target chamber. Bottom panel: measured absorbance $A$ of argon.}
