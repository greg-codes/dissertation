...
$$
\Delta y_p \approx 2 \times 0.2941 \frac{\lambda_1 f}{\sigma}
$$
For $\lambda = 1350 \ \textrm{nm}$, $f=40 \ \textrm{cm}$ and $\sigma = 10 \ \textrm{mm}$, the IR spot separation is about 31.8 $\mu$m. By placing a gas jet at the IR focus, each intensity lobe will locally drive the HHG process. As a result, we will have two spatially separated phase-locked XUV light sources that will interfere in the far field with a spatial frequency $\tilde{k}_q$  \cite{camperTransverseElectromagneticMode2015}:
\begin{equation}
\tilde{k}_q = q \frac{2 \pi a}{\lambda_1 D}
\end{equation}
where $D$ is the source-to-screen distance, $a$ is the distance between the two sources, $\lambda_1$ is the fundamental wavelength. 
