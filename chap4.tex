...
A_{\textrm{e.s.}} &= \alpha_0 L + \alpha_1 L \bar{\eta}_z(x,y).
\end{aligned}
\end{equation}
...
\begin{equation}
\Delta A = -\log_{10} \left( \frac{ \iint \dd{x} \dd{y} I_{\textrm{XUV}}(x,y) 10^{- \alpha_1 L \bar{\eta}_z(x,y)}}{ \iint \dd{x} \dd{y} I_{\textrm{XUV}}(x,y)} \right).
\label{eqn:Delta_A_3}
\end{equation}
...
\begin{figure}
	\centering
	\includegraphics[width=1.0\textwidth]{figures/chap4/Channel_VB_resolved.pdf}
	\caption{Transition type and initial band resolved contributions to the final conduction band density. See text for details.}
	\label{fig:Channel_VB_resolved}
	% plot made with \Python Scripts\DrakeIonization\DrakeIonization.py
\end{figure}

\cref{fig:Channel_VB_resolved} shows the main result of the Keldysh calculation. We plot the contributions from each ionization channel (direct \& indirect) and for each of the three initial bands after the IR pulse has left ($\tau = 120 \ \textrm{fs}$) as a function of peak intensity. We see an intensity-dependent effect: $N_{\textrm{CB}}$ has a local maximum, a slight dip, and then an increase. At this scale, this feature is most prominent in the indirect heavy hole plot, but the general shape is the same for all channels. We interpret this as a result of the effective bandgap changing with increasing field strength (see \cref{eqn:eff_bandgap_HHLL,eqn:eff_bandgap_SO}).
...
\begin{figure}
	...
	\caption{Calculated final $\Delta A$ as a function of input pulse energy. These curves were calculated using $\alpha_1=1$ (absorption). See text for details.}
...
\begin{figure}
	...
	\caption{Calculated $\Delta A$ as a function of pulse delay time, $\tau$. This was calculated using $\alpha_1=1$ (absorption).}
